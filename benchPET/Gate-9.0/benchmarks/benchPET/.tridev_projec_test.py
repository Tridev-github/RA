...
def analyse_pet(output_folder, ax, i):
    ...
    tuc = z[mask]
    tsc = z[~mask]
    print(tscattered, len(tsc))
    print(tunscattered, len(tuc))
    a = ax[i+0,2]
    a.hist(tuc, bins=100)
    a.set_xlabel('mm')
    a.set_ylabel('counts')
...
def plot_all(output_folders, ax):
    ...
    for o, i in zip(output_folders, range(len(output_folders))):
        ...
        test = (0.99*previousDictTest[coincidences] <= dictTest[coincidences] <= 1.01*previousDictTest[coincidences]) and                (0.99*previousDictTest[true]         <= dictTest[true]         <= 1.01*previousDictTest[true]) and                (0.95*previousDictTest[random]       <= dictTest[random]       <= 1.05*previousDictTest[random]) and                (0.99*previousDictTest[scatter]      <= dictTest[scatter]      <= 1.01*previousDictTest[scatter]) and                (0.90*previousDictTest[delay]        <= dictTest[delay]        <= 1.10*previousDictTest[delay]) and                (0.95 * 122.24                         <= dictTest[hl]           <= 1.05 * 122.24)
